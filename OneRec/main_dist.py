...
def get_args():
    ...
    # Train params
    parser.add_argument('--single_device_batch_size', default=256, type=int)
    ...
    parser.add_argument('--hash_emb_size', default=64, type=int)

    # hstu
    parser.add_argument('--use_hstu', action='store_true', default=False,
                        help='Use HSTU blocks instead of standard Transformer blocks.')
    ...
    parser.add_argument('--dnn_hidden_units', default=8, type=int)  # 设置模型的dnn是多少倍hidden units
    ...
    # "sid"
    parser.add_argument('--item_sparse', default=['exposure_start_year', 'exposure_start_month', 'exposure_start_day','exposure_end_year', 'exposure_end_month', 'exposure_end_day'],
    ...
    # 'next_action_type'
    # 'timestamp_bucket_span'
    parser.add_argument('--context_item_sparse',
                        default=['time_diff_day', 'time_diff_hour', 'time_diff_minute','timestamp_bucket_id','hot_bucket_1000'], type=str,
                        nargs='+')
    parser.add_argument('--feature_dropout_list', default=['timestamp_bucket_id','hot_bucket_1000'], type=str, nargs='+')
    ...
    # sid
    parser.add_argument('--sid', action="store_true", default=True)
    ...
    parser.add_argument('--reward', default=False)
    ...
    parser.add_argument('--train_infer_result_path', type=str, default='sid_beam_1027')
    ...
    parser.add_argument('--moe_load_balancing_alpha', default=0.01, type=float)
    ...
    parser.add_argument('--moe_sequence_aux_loss_coeff', default=0.01, type=float,
                        help='Coefficient for sequence-level auxiliary loss')
    # ====== 新增：SID Beam Search 生成模式 (用于 train_infer.py) ======
    parser.add_argument('--beam_search_generate', action="store_true", default=True,
                        help='(train_infer.py) 启用 SID beam search 批量生成模式')
    parser.add_argument('--beam_search_top_k', default=384, type=int,
...
def evaluation(args, model, valid_loader, writer, epoch, global_step, dataset, device):
    ...
    if 'loss_MLP_AUC/train' in final_metrics:
        mlp_auc = final_metrics['loss_MLP_AUC/train']
        print(f"Epoch {epoch} AUC: MLP={mlp_auc:.4f}", end="")
        writer.add_scalar('AUC/MLP/valid', mlp_auc, global_step)

        if 'loss_ANN_AUC/train' in final_metrics:
            ann_auc = final_metrics['loss_ANN_AUC/train']
            print(f", ANN={ann_auc:.4f}")
            writer.add_scalar('AUC/ANN/valid', ann_auc, global_step)
        else:
            print()  # 换行
    if 'loss_SID/Top10HitRate1'in final_metrics:
        sid_auc1 = final_metrics['loss_SID/Top10HitRate1']
        print(f"Epoch {epoch} AUC: SID1={sid_auc1:.4f}", end="")
        writer.add_scalar('SID/valid1', sid_auc1, global_step)
        sid_auc2 = final_metrics['loss_SID/Top10HitRate2']
        print(f", SID2={sid_auc2:.4f}")
        writer.add_scalar('SID/valid2', sid_auc2, global_step)
    # 记录损失指标
    if 'loss_total' in final_metrics:
        ...
        if 'loss_reward' in final_metrics:
            writer.add_scalar('Reward/valid', final_metrics['loss_reward'], global_step)
    else:
        valid_loss = 0.0
...
def train(args, run_name="default_run"):
    ...
    if args.use_preprocessing:
        ...
        train_loader = DynamicPreprocessedDataLoader(
            ...
            max_batches=real_max_batches, wait_timeout=300, batch_size=args.batch_size
        )
    ...
    if args.pure_bf16:
        ...
        if model.use_moe and hasattr(model, 'hstu_layers'):
            # 遍历每一个 HSTU block，因为每个 MoE 层都有自己的负载均衡策略
            for block in model.hstu_layers:
    ...
    if not args.rope and not args.use_hstu:
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        sid1_loss_sum = 0.0
        sid2_loss_sum = 0.0
        mlp_bce_loss_sum = 0.0
        moe_aux_loss_sum = 0.0
        mlp_auc_sum = 0.0
        ann_auc_sum = 0.0
        auc_count = 0  # 因为AUC可能不是每步都能计算，所以需要一个单独的计数器
        pos_sim_sum = 0.0
        loss_count = 0
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            if 'ctr_bce/train' in train_log_dict:
                infonce_sum += train_log_dict['ctr_bce/train']
            ...
            if args.sid:
                ...
                if "auc_type1" in train_log_dict:
                    writer.add_scalar('Action_AUC/auc_type1', float(train_log_dict['auc_type1']), global_step)
                if "auc_type2" in train_log_dict:
                    writer.add_scalar('Action_AUC/auc_type2', float(train_log_dict['auc_type2']), global_step)

            if args.reward:
                if 'MLP_BCE_Loss/train' in train_log_dict:
                    mlp_bce_loss_sum += train_log_dict['MLP_BCE_Loss/train']
                if 'MLP_AUC/train' in train_log_dict:
                    mlp_auc_sum += train_log_dict['MLP_AUC/train']
                    auc_count += 1
                if 'ANN_AUC/train' in train_log_dict:
                    ann_auc_sum += train_log_dict['ANN_AUC/train']
            ...
            if step == 0:
                ...
                if args.sid:
                    ...
                    # 添加SID top10命中率列表
                    sid_top10_hitrate1_list, sid_top10_hitrate2_list = [], []

            # 2. 累加 Loss 和 计数器 (在每个 step 都要执行，不仅仅是 args.sid 为真时)
            #    注意：这里假设 train_log_dict 包含了当前 step 的 loss 值
            #    如果 loss 变量在外部定义 (如 loss_sid1), 请确保这里传入的是 .item() 标量
            loss_count += 1

            # 3. SID 指标记录与 TensorBoard 日志写入
            if args.sid:
                # --- [修改] 获取当前 batch 的指标 ---
                current_sid_hitrate1 = train_log_dict.get('SID/Top10HitRate1', 0.0)
                current_sid_hitrate2 = train_log_dict.get('SID/Top10HitRate2', 0.0)

                # 将日志字典中的值更新为当前 Batch 的值，而不是平均值
                log_dict['SID/Top10HitRate1'] = current_sid_hitrate1
                log_dict['SID/Top10HitRate2'] = current_sid_hitrate2

                # 仅在主进程进行 TensorBoard 写入
                if multiprocessing.current_process().name == 'MainProcess':
                    # --- [关键修改] 全部记录当前 Batch 的值 ---

                    # 1. 记录 SID HitRate (当前 Batch)
                    writer.add_scalar('SID/Top10HitRate1', current_sid_hitrate1, global_step)
                    writer.add_scalar('SID/Top10HitRate2', current_sid_hitrate2, global_step)

                    # 2. 记录 SID Loss (当前 Batch)
                    # 注意：直接使用 train_log_dict 中的 key，不要使用累加器除以 count
                    cur_sid1_loss = train_log_dict.get('Sid1Loss/train', 0.0)
                    cur_sid2_loss = train_log_dict.get('Sid2Loss/train', 0.0)

                    writer.add_scalar('Sid1Loss/train_batch', cur_sid1_loss, global_step)
                    writer.add_scalar('Sid2Loss/train_batch', cur_sid2_loss, global_step)

                    # 3. 记录 Action Loss (当前 Batch)
                    # 对应 model.py 中 log_dict['CTR_BCE_Loss/train']
                    cur_action_loss = train_log_dict.get('CTR_BCE_Loss/train', 0.0)
                    writer.add_scalar('ActionLoss/train_batch', cur_action_loss, global_step)
            ...
            if 'Similarity/positive_train' in train_log_dict:
                ...
                if args.sid:
                    ...
                    # 添加SID top10命中率监控
                    sid_top10_hitrate1 = train_log_dict.get('SID/Top10HitRate1', 0.0)
                    sid_top10_hitrate2 = train_log_dict.get('SID/Top10HitRate2', 0.0)
                if step == 0:
                    hr10_list, ndcg10_list, score_list = [], [], []
                    hr10_last_list, ndcg10_last_list, score_last_list = [], [], []
                    if args.sid:
                ...
                if args.sid:
                    ...
                    # 收集SID top10命中率数据
                    sid_top10_hitrate1_list.append(sid_top10_hitrate1)
                    sid_top10_hitrate2_list.append(sid_top10_hitrate2)
                ...
                if args.sid:
                    ...
                    # 计算SID top10命中率平均值
                    sid_top10_hitrate1_avg = sum(sid_top10_hitrate1_list) / len(sid_top10_hitrate1_list)
                    sid_top10_hitrate2_avg = sum(sid_top10_hitrate2_list) / len(sid_top10_hitrate2_list)
                    ...
                    # 添加SID top10命中率到日志
                    log_dict['SID/Top10HitRate1'] = sid_top10_hitrate1_avg
                    log_dict['SID/Top10HitRate2'] = sid_top10_hitrate2_avg
                    if multiprocessing.current_process().name == 'MainProcess':
                        ...
                        # 添加SID top10命中率到TensorBoard
                        writer.add_scalar('SID/Top10HitRate1', sid_top10_hitrate1_avg, global_step)
                        writer.add_scalar('SID/Top10HitRate2', sid_top10_hitrate2_avg, global_step)


                if args.reward:
                    sid_reward_hr = train_log_dict.get('Reward/RewardHR@10/train', None)
                    sid_reward_ndcg = train_log_dict.get('Reward/RewardNDCG@10/train', None)
                    sid_reward_score = train_log_dict.get('Reward/RewardScore/train', None)


                    if multiprocessing.current_process().name == 'MainProcess':
                        # SID Reward Model 指标监控
                        if sid_reward_hr is not None:
                            # 同步写入 log_dict，便于 JSON 日志记录
                            log_dict['Reward/RewardHR@10/train'] = sid_reward_hr
                            log_dict['Reward/RewardNDCG@10/train'] = sid_reward_ndcg
                            log_dict['Reward/RewardScore/train'] = sid_reward_score
                            # 写入 TensorBoard
                            writer.add_scalar('Reward/RewardHR@10/train', sid_reward_hr, global_step)
                            writer.add_scalar('Reward/RewardNDCG@10/train', sid_reward_ndcg, global_step)
                            writer.add_scalar('Reward/RewardScore/train', sid_reward_score, global_step)

                # --- 新增的平均指标写入 ---
                if multiprocessing.current_process().name == 'MainProcess':
                    writer.add_scalar('Loss/train_avg', loss_sum / loss_count, global_step)
                    if args.infonce: writer.add_scalar('ctr_bce/train_avg', infonce_sum / loss_count, global_step)
                    if args.sid:
                    ...
                    if args.reward:
                        writer.add_scalar('MLP_BCE_Loss/train_avg', mlp_bce_loss_sum / loss_count, global_step)
                        if auc_count > 0:
                            writer.add_scalar('AUC/MLP', mlp_auc_sum / auc_count, global_step)
                            writer.add_scalar('AUC/ANN', ann_auc_sum / auc_count, global_step)
                ...
                if args.sid:
                    ...
                    # 清空SID top10命中率列表
                    sid_top10_hitrate1_list.clear()
                    sid_top10_hitrate2_list.clear()
            ...
            # # 调试：检查各个GPU上的loss情况
            # if args.use_my_dataparallel and hasattr(model, 'replicas'):
            #     print(f"Number of replicas: {len(model.replicas)}")
            #     for i, replica in enumerate(model.replicas):
            #         print(f"Replica {i} device: {next(replica.parameters()).device}")
            #         # 检查replica是否在正确的设备上
            #         if hasattr(replica, 'dev'):
            #             print(f"Replica {i} dev attribute: {replica.dev}")

            #     # 检查当前batch的device
            #     print(f"Current batch device: {seq.device}")
            #     print(f"Loss device: {loss.device}")

            #     # 检查模型参数是否分布在多个GPU上
            #     for i, replica in enumerate(model.replicas):
            #         first_param = next(replica.parameters())
            #         print(f"Replica {i} first parameter device: {first_param.device}")
            #         break  # 只检查第一个参数作为示例
        ...
        valid_loss, score = evaluation(args, model, valid_loader, writer, epoch, global_step, dataset,
                                       device=args.device)

        #if args.test_train_valid_process:
            #break

        save_dir = Path(args.ckpt_path,
                        f"global_step{global_step}.valid_loss={valid_loss:.4f}.score={score:.4f}")
