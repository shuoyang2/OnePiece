...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        self.item_feat_dict = json.load(open(Path(data_dir, "item_feat_dict.json"), 'r'))  # reid: id feature
        self.user_action_type = orjson.loads(open(self.data_dir / "user_action_type.json", 'rb').read())
        self.force_full_seq = True
    ...
    def __getitem__(self, uid):
        ...
        for record_tuple in reversed(ext_user_sequence):
            ...
            # user,item1, item2, ..., item_n
            # item1, item2,...,item_n

            if next_act_type is not None:
                next_action_type[idx] = next_act_type
            else:
                if idx == self.maxlen:  # 如果是最后一个位置，则需要查询user_action_type
            ...
            if next_timestamp is not None:
                ...
                ranking_loss_mask[idx] = 0  # 没有 Target，当然不计算 Loss
            ...
            if self.sid and type_ == 1:  # 仅针对 Item (type_==1) 填充 SID
                c_i = self.indexer_i_rev[i]
                if c_i in self.sid:
